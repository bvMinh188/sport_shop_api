...
describe('Test Case: Hủy đơn hàng ở trạng thái "Đã xác nhận"', function () {
    let driver;
    ...
    it('TC5: Hủy đơn hàng ở trạng thái "Đã xác nhận"', async function () {
        this.timeout(20000);
        ...
        try {
            ...
            for (let i = 0; i < orderList.length; i++) {
                ...
                if (status.includes('Đã xác nhận') || status.includes('Confirmed') || status.includes('Approved')) {
                    orderToCancel = order;
            ...
            await driver.wait(until.elementLocated(By.css('.message, .alert, .toast, .error-message')), 5000);
            
            const message = await driver.findElement(By.css('.message, .alert, .toast, .error-message')).getText();
            expect(message).to.include('không thể hủy') || expect(message).to.include('cannot cancel') || expect(message).to.include('not allowed') || expect(message).to.include('error');
            ...
            expect(updatedStatus).to.include('Đã xác nhận') || expect(updatedStatus).to.include('Confirmed') || expect(updatedStatus).to.include('Approved');
            
        } catch (error) {
            try {
                const screenshot = await driver.takeScreenshot();
                require('fs').writeFileSync('cancel-confirmed-order-failure.png', screenshot, 'base64');
